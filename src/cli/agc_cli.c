...
static void CliShowUsage(void)
{
  printf(
    ...
    "interface.\n"
    "--symbols=SYMFILE        Read symbols from SYMFILE generated by "
    "yaYUL.\n"
    ...
    "--port=N                 Change the server port number "
    "(default=19697).\n"
    "--nodebug                Disables debugging and run just the "
    "simulation\n"
    "--interlace=N            Read the socket interface every N CPU "
    "instructions.\n"
    "--dump-time=N            Create core image every N seconds "
    "(default = 10).\n"
    "--cdu-log                Used only for debugging. Creates the "
    "file yaAGC.cdulog\n"
    "                         containing data related to the "
    "bandwidth-limiting of\n"
    "                         CDU inputs PCDU and MCDU.\n"
    "--debug-dsky             Rather than run the core program, go "
    "into DSKY-debug\n"
    "                         mode. In this mode send pre-determined "
    "codes to\n"
    "                         the DSKY upon receiving DSKY "
    "keypresses.\n"
    "--debug-deda             This mode runs the core program as "
    "usual, but also\n"
    "                         responds to messages from yaDEDA and "
    "generates fake\n"
    "                         messages to yaDEDA for testing "
    "purposes.\n"
    "--deda-quiet             Used with --debug-deda to eliminate "
    "outputs from yaAGC\n"
    "                         to the DEDA.  In other words, lets "
    "yaAGC parse the\n"
    "                         messages being received from the DEDA, "
    "but never to\n"
    "                         send any.  That lets \"yaAGC "
    "--debug-deda --deda-quiet\"\n"
    "                         to be used alongside yaAGS without "
    "conflict.\n"
    "--inhibit-alarms         Prevents the simulated hardware alarms "
    "(Night Watchman\n"
    "                         Rupt Lock, and TC Trap) from causing "
    "resets.\n"
    "--show-alarms            Prints notifications upon hardware "
    "alarm (Night Watchman\n"
    "                         Rupt Lock, and TC Trap).\n"
    "--cfg=file               The name of a configuration file.  "
    "Presently, the\n"
    "                         configuration files is used only for "
    "--debug-dsky\n"
    "                         mode.  It would typically be the same "
    "configuration\n"
    "                         file as used for yaDSKY.\n\n"
    "--initialize-sunburst-37 Makes some initializations to erasable "
    "memory needed\n"
    "                         for a first-time run of SUNBURST 37 "
    "(SHEPATIN 0) having\n"
    "                         otherwise clean memory, in lieu of "
    "pad-loads.  Not\n"
    "                         required for subsequent runs.  Note "
    "that this option only\n"
    "                         has an effect if there is no existing "
    "core-dump file.\n"
    "--no-resume              Disables the resuming from a "
    "core-resume-file.\n"
    "                         By default yaAGC resumes from the "
    "\"core\" resume file or\n"
    "                         another file provided as a command "
    "line argument.\n"
    "Note that the exec-ropes file should contain exactly 36 banks\n"
    "(36x1024=36864 words, or 73728 bytes). Other sizes may be "
    "accepted,\n"
    "but it is unclear what (if any) utility such core-rope images\n"
    "would have. (In particular, if the core-rope\n"
    "is supposed to be for actual Luminary or Colossus software, "
    "then\n"
    "the checksums of the missing memory banks would be incorrect, "
    "and\n"
...
static int CliProcessArgument(char* token)
{
  ...
  else if(!strcmp(token, "-show-alarms"))
    Options.show_alarms = 1;
  ...
  else if(!strcmp(token, "-no-resume"))
    Options.no_resume = 1;
